...
class ChainalysisCotroller extends \schema\controller\MainController
{
    ...
    private function getTransactions($inputAddress, $outputAddress, $transactioncount)
    {
        ...
        $this->conn->add("SELECT a.block_id, a.spending_transaction_hash, b.transaction_hash, a.time, b.value_usd, a.recipient as sender, b.recipient as receiver");
        $this->conn->add("FROM inputs a INNER JOIN outputs b ON a.spending_transaction_hash = b.transaction_hash" );
        ...
        if (!empty($inputAddress) && !empty($outputAddress)) {
            ...
            $this->conn->add("WHERE (a.recipient LIKE '{$inputAddress}%')  {$additionalCondition}  GROUP BY (a.block_id,  a.spending_transaction_hash, b.transaction_hash, a.time, b.value_usd, a.recipient, b.recipient) LIMIT {$transactioncount}");
            // echo "WHERE (a.recipient LIKE '{$inputAddress}%')  {$additionalCondition} LIMIT {$transactioncount}";
        } elseif (empty($inputAddress) && !empty($outputAddress)) {
            $this->conn->add("WHERE b.recipient LIKE '{$outputAddress}%' GROUP BY (a.block_id,  a.spending_transaction_hash, b.transaction_hash, a.time, b.value_usd, a.recipient, b.recipient) LIMIT {$transactioncount}");
            // echo "WHERE (a.recipient LIKE '{$outputAddress}%')  {$additionalCondition} LIMIT {$transactioncount}";
        } else {
            $this->conn->add("WHERE 1=1 {$additionalCondition}  GROUP BY (a.block_id,  a.spending_transaction_hash, b.transaction_hash, a.time, b.value_usd, a.recipient, b.recipient) LIMIT {$transactioncount}");
        }
